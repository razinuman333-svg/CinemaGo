...
function SeatLayout() {
  ...
  return show ? (
   ...
   <div className=' w-60 bg-primary/10 border border-primary/20 rounded-lg py-10 h-max md:sticky md:top-30'>
    ...
    <div className='mt-5 space-y-1'>
      ...
      {show.dateTime[date].map((item,index)=>(
        <div onClick={()=>setSelectedTime(item)} key={index} className={` flex items-center gap-2 px-6 py-2 w-max rounded-r-md cursor-pointer transition
   ...
   <div className=' relative  flex-1 flex flex-col items-center max-md:mt-16'>
    ...
    <img className='w-90 h-20' src={assets.screen}/>
    <p>Screen Side</p>
   ...
   <div className='grid grid-cols-2 gap-11'>
      {groupRows.slice(1).map((item,index)=>(
        <div key={index} className=''>
         {item.map((row)=>renderSeats(row))}
        </div>
